# 🛍️ Приложение с использованием DeepSeek-R1 для анализа отзывов на маркетплейсах
...
- ⚙️ Кросс-платформенная поддержка (Windows, Linux, Mac с настройкой путей)
- 🖥️ Частично оффлайн, без API или облачных запросов
...
> ❗ Убедись, что установлена .NET 8 SDK: https://dotnet.microsoft.com/en-us/download
...
Не забудьте положить chromedriver.exe в папку chromedriver/ или установить пакет Selenium.WebDriver.ChromeDriver(подтянется с dotnet restore) и использовать путь по умолчанию
...
> ⚠️ Если хотите использовать GPU (CUDA), поменяйте LLamaSharp.Backend.Cpu на соответствующий GPU-бэкенд, а также измените параметры в appsettings.json
...
- [DeepSeek](https://deepseek.com/) — мощная open-source модель
- [LLamaSharp](https://github.com/SciSharp/LLamaSharp) — .NET-обертка над llama.cpp